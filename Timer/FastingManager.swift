...
class FastingManager: ObservableObject {
    @Published private(set) var festingState: FastingState = .notStarted
    ...
    @Published private(set) var elapesedTime: Double = 0.0
...
//        print("scheduledTime", scheduledTime.formatted(.dateTime.month().day().hour().minute().second()))
    ...
    func toggleFastingState() {
        festingState = festingState == .fasting ? .feeding : .fasting
        startTime = Date()
        elapesedTime = 0.0
    }
    
    func track() {
        guard festingState != .notStarted else { return }
        ...
        elapesedTime += 1
        print("elapsedTime", elapesedTime)
        
        let totalTime = festingState == .fasting ? fastingTime : feedingTime
        progress =  (elapesedTime / totalTime * 100).rounded() / 100
